...
LANDMARK_LIB_PATH = os.path.join(VOVIT_LIB_PATH, "3DDFA_V2_python11")
...
class BaseDict(dict):
    def __add__(self, other):
        ...
        for key in self.keys():
            if key in o_keys:
                raise KeyError(
                    "Cannot concatenate both dictionaries. Key %s duplicated" % key
                )
        self.update(other)
        return self
...
def get_duration_fps(filename, display):
    ...
    result = subprocess.Popen(
        ["ffprobe", str(filename)], stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
...
def np_int2float(waveform: np.ndarray, raise_error: bool = False) -> np.ndarray:
    ...
    if waveform.dtype == np.int8:
        ...
        raise TypeError(
            f"int2float input should be of type np.intXX but {waveform.dtype} found"
        )
    else:
        return waveform
...
def process_video(
    ...
    assert landmark_type in ["2d_sparse", "2d_dense", "3d"], (
        f"Landmarks should be either 2d_sparse, 2d_dense or 3d "
    ...
    if save_video:
        ...
        assert os.path.exists(
            basename
        ), f"Basename of video_dst {video_dst} does not exist"
    if save_landmarks:
        ...
        assert os.path.exists(
            basename
        ), f"Basename of video_dst {landmarks_dst} does not exist"
    if save_metadata:
        ...
        assert os.path.exists(
            basename
        ), f"Basename of video_dst {metadata_dst} does not exist"
    ...
    if assert_fps is not None:
        assert (
            fps == assert_fps
        ), f"FPS required to be {assert_fps} but video is {fps} FPS"
    if save_video:
        ...
        if len(queue_ver) >= n:
            ...
            if save_video:
                if landmark_type == "2d_sparse":
                    ...
                    img_draw = render(
                        queue_frame[n_pre], [ver_ave], tddfa.tri, alpha=0.7
                    )
                else:
    ...
    if assert_frames:
        if abs(expected_frames - i) > 5:
            print(
                f"A duration of {duration} at {fps} fps implies {expected_frames} frames "
